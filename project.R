...
nonCarnivores_trilobiteEx = dat[dat$diet != 'carnivore' | dat$class == "Trilobita",]
...
full_tsplot <- function(x=NULL, ys=NULL, cols=c(carniCol, nonCarniCol), labels=c("Carnivores", "Other"),
                           xlim = 4:95, ylim = c(0, 1),
                           ylab = "Diversity Proportion",
                           boxes = "sys", shading = "sys",
                           event_lines = c(66, 201, 252, 372, 445),
                          leg = TRUE,
                           legend_pos = "topleft", legend_inset = c(0.05, 0.05),
                           legend_cex = 1.3,
                           tit="Diversity") {
...
# diversity rates
full_tsplot(x=allDiv$mid, ys=list(carniDiv$divSIB, nonCarniDiv$divSIB), 
            tit="Diversity",
            ylab="SIB Diversity",
            ylim=c(0, max(carniDiv$divSIB, nonCarniDiv$divSIB, na.rm = TRUE)))
...
prop_tsplot = function(proportion, tit="Relative Makeup of Diets") {
  ...
  legend("topleft", bg = "white", legend = c("Carnivores", "other"), title="Diet",
         col = c(rgb(1, 0, 0), rgb(0, 0, 0)), lwd = 2, inset = c(0.05, 0.05),
         cex = 1.3,)
...
full_tsplot(x=allDiv$mid, ys=list(carniDiv$extPC, nonCarniDiv$extPC), 
            tit="Extinctions",
            ylab="Foote Metric",
            ylim=c(0, max(max(carniAmmonExDiv$extPC, na.rm=TRUE), 
                          max(noncarniAmmonExDiv$extPC, na.rm=TRUE),
                          max(carniTriloExDiv$extPC, na.rm=TRUE),
                          max(noncarniTriloExDiv$extPC, na.rm=TRUE)))
            )
# add ammonite lines
lines(x=allDiv$mid, y=carniAmmonExDiv$extPC, lwd = 2, col = carniCol, type = "l", lty=2)
lines(x=allDiv$mid, y=noncarniAmmonExDiv$extPC, lwd = 2, col = nonCarniCol, type = "l", lty=2)
legend("topright", bg = "white", legend = c("Default", "Ammonites not carnivore"), lwd = 2, lty=c(1, 2),inset = c(0.05, 0.05), cex = 1.3)

# add trilobite lines
lines(x=allDiv$mid, y=carniTriloExDiv$extPC, lwd = 2, col = carniCol, type = "l", lty=3)
lines(x=allDiv$mid, y=noncarniTriloExDiv$extPC, lwd = 2, col = nonCarniCol, type = "l", lty=3)
legend("topright", bg = "white", legend = c("Default", "Ammonites not carnivore", "Trilobites not carnivore"), lwd = 2, lty=c(1, 2, 3),inset = c(0.05, 0.05), cex = 1.3)




# total corrected
full_tsplot(x=allDiv$mid, ys=list(carniDiv$ext2f3, nonCarniDiv$ext2f3), 
            tit="Extinctions",
            ylab="Foote Metric", ylim = c(0,2))

# relative extinction rates
full_tsplot(x=allDiv$mid, ys=list(carniDiv$extProp, nonCarniDiv$extProp), 
                        tit="Extinctions",ylab="Proportion")
full_tsplot(x=allDiv$mid, ys=list(carniAmmonExDiv$extProp, noncarniAmmonExDiv$extProp), 
            tit="Extinctions (ammonites counted as non-carnivores)",
            ylab="Proportion")
full_tsplot(x=allDiv$mid, ys=list(carniTriloExDiv$extProp, noncarniTriloExDiv$extProp), 
            tit="Extinctions (ammonites counted as non-carnivores)",
            ylab="Proportion")
...
full_tsplot(allDiv$mid, ys=list(carniExtZscore, nonCarniExtZscore), ylim=c(-0.5, 1.5), tit="Extinction Rates",ylab="Foote Metric (Z-Scored)")

# difference between zscores
carniExtDiffZscore = carniExtZscore - nonCarniExtZscore
full_tsplot(allDiv$mid, ys=list(carniExtDiffZscore), ylim=c(-0.5,1.5), tit="Difference between Carnivore Extinctions and others",ylab="Foote Metric (Z-Scored)",  cols=c("brown1"), labels=c("Carnivores - Non-Carnivores"))
